	...
	private void backtracking(int level) {
		if (level == numberOfTotalCharacters) {
			...
			if (level < numberOfTotalCharacters - numberOfNonLettersEnds) {
				//if level == 3; numberOfTotalCharacters = 10; numberOfNonLettersEnd = 2;
				Collections.shuffle(lettersList);
				elements = (Character[])lettersList.toArray();
			} else { //I need to include a non-letter element
				//if level == 9; numberOfTotalCharacters = 10; numberOfNonLettersEnd = 2;
