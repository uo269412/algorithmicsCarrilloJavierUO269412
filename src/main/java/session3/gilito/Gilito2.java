...
public class Gilito2 extends Gilito1 {
	...
	public int calculate() {
		return calculateRec(0, (getCoinsLength() - 1) / 2, getCoinsLength() / 2, getCoinsLength() - 1);
	}

	private int calculateRec(int leftMin, int leftMax, int rightMin, int rightMax) {
		...
		if (res == 1) { // less heavy on left
			...
			return ((rightMax + 1) / 2 % 2 != 0) // Case given when the sample divided by two is not a even number
					? calculateRec(leftMin, (leftMax + leftMin) / 2, (leftMax + leftMin) / 2, leftMax)
					: calculateRec(leftMin, (leftMax + leftMin - 1) / 2, (leftMax + 1 + leftMin) / 2, leftMax);

		} else if (res == 2) { // less heavy on right
			if (rightMin == (rightMin + rightMax - 1) / 2) { // When the sample is small, we get the coin
			...
			return ((rightMax + 1) / 2 % 2 != 0) // Case given when the sample divided by two is not a even number
					? calculateRec(rightMin, (rightMin + rightMax) / 2, (rightMin + rightMax) / 2, rightMax)
					: calculateRec(rightMin, (rightMin + rightMax - 1) / 2, (rightMin + 1 + rightMax) / 2, rightMax);
		}
		return rightMin;
	}
	...
	private int calculateUnderstandable(int leftMin, int leftMax, int rightMin, int rightMax) {
		...
		if (res == 1) {
			...
			if ((rightMax + 1) / 2 % 2 != 0) {
				...
				return calculateUnderstandable(leftMin, (leftMax + leftMin - 1) / 2, (leftMax + 1 + leftMin) / 2,
						leftMax);
			}
			...
			if ((rightMax + 1) / 2 % 2 != 0) {
				return calculateUnderstandable(rightMin, (rightMin + rightMax) / 2, (rightMin + rightMax) / 2,
						rightMax);

			} else {
				return calculateUnderstandable(rightMin, (rightMin + rightMax - 1) / 2, (rightMin + 1 + rightMax) / 2,
						rightMax);
			}
...
//		   gilito2.setCoinWeight(3, FAKE_WEIGHT); 
...
//		gilito2.setCoinWeight(8, FAKE_WEIGHT);
//		System.out.println("RESULT: " + gilito2.calculate());
